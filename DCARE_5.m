function DCARE_5
%% Extract hbr time series from dc variable in subject file
...
%%  find period of interest
    ...
    [~,period,~,~,~]=wtc(hbr1,hbr2,'mcc',0,'ms',128); 
...
%% calculate coherences for every channel
    for ch=1:16
        hbr1=[t, hbr_ch(:,ch)];
        hbr2=[t, hbr_cg(:,ch)];
        [Rsq, period,coi]=wtc(hbr1,hbr2,'mcc',0,'ms',128);
...
save(sprintf(['P:/projects/DCARE/DCARE/MATLAB/procData/wtcData/DCARE_%02d_wtc_hbr.mat'],dyad),'wtc_all', 'Rsq')
end
end