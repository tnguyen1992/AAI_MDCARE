...
for i = 1:1:numOfSources
    for n=1:1:numOfPermutations
        ...
        hbrSub2 = data_preproc.sub2.hbr;

        % Adjust both files to the shortest length
        minLen = min(length(hbrSub1), length(hbrSub2));
        hbrSub1 = hbrSub1(1:minLen,1:16);
        hbrSub2 = hbrSub2(1:minLen,1:16);

        t = (0:(1/data_preproc.sub1.fs):((size(hbrSub1, 1) - 1) / ...
            data_preproc.sub1.fs))';

        numOfChan = size(hbrSub1, 2);
        ...
        pnoi = zeros(2,1);
        ii = numOfChan;
        while (isnan(hbrSub1(1, ii)) || isnan(hbrSub2(1, ii)))                        % check if 16 th channel was not rejected in both subjects during preprocessing
        ...
        if ii ~= 0
            sigPart1 = [t, hbrSub1(:,ii)];
            sigPart2 = [t, hbrSub2(:,ii)];
        ...
        for ii=1:1:numOfChan
            if ~isnan(hbrSub1(1, ii)) && ~isnan(hbrSub2(1, ii))                         % check if this channel was not rejected in both subjects during preprocessing
                sigPart1 = [t, hbrSub1(:,ii)];
                sigPart2 = [t, hbrSub2(:,ii)];
    ...
    data_wtc.hbr.coherence_m       = coherences_m;
    data_wtc.hbr.coherence       = coherences;
    %     data_wtc.Rsq                  = Rsq;
    data_wtc.paramStrings         = {'Cooperation', 'Individual', 'Rest'};
    data_wtc.channel              = 1:1:size(hbrSub1, 2);
    data_wtc.t                    = t;
    ...
    cfg.filename    = strcat(filenameWithoutMat,'_rpa_hbr.mat');
...
for id = numOfPart
  ...
  RPAFile = strcat('D:/InvTie/RPA/', sprintf(['MCARE_d%02d_02a_preproc_002_rpa_hbr.mat.mat'],id));
  load(RPAFile)
	...
	for condition=1:1:3
        for channel=1:1:16
                ...
                rpa(i,2)=condition;
                rpa(i,3)=channel;
                rpa(i,4)=data_wtc.hbr.coherence_m(condition, channel);
                i=i+1;
        end
    end
...
for id = numOfPart
  ...
  RPAFile = strcat('D:/InvTie/RPA/', sprintf(['DCARE_d%02d_02a_preproc_001_rpa_hbr.mat.mat'],id));
  load(RPAFile)
...
dlmwrite('MDCARE_AAI_rpa_hbr.csv', rpa)